...
/**
 * HTMLLexer is a mutable object that
 * generates a stream of tokens form s string in
 * HTML format
 */
public class HTMLLexer {
	...
	// s is the string of HTML text that we're parsing,
	...
	private static final Pattern TOKEN_REGEX
		= Pattern.compile(
				"^"			// anchors to the beginning of the match start,
							// so that we don't skip any characters
				+ "(<i>)"	// START_I
				+ "|"
				+ "(</i>)"	// END_I
				+ "|"
	...
	private static final Type[] TOKEN_TYPES
		= {Type.START_I, Type.END_I, Type.TEXT};
	...
	public Token next() throws SyntaxErrorException {
		...
		// Only one of the group can match, so look for a non-null group.
		for (int i = 1; i < matcher.groupCount(); i = i + 1) {
			if (matcher.group(i) != null) {
				// since i is 1-based, use i-1 to find the token type
				// for this pattern
				return new Token(TOKEN_TYPES[i-1], value);
			}
		}
		
		// if we got here, thers's a bug in the regex -- Matcher said
		// we matched the expression, but it didn't match any of the
		// parens
