 ...
 * italic text (generates a random anagram of each italic part) so that
 * Kepler can't read it.
 */
public class Gallileo {
	 ...
	 * of the production.
	 * @param tok Token that started this process, required to be TEXT
	 * @return evaluated string
	 */
	private String evalNormal(Token tok) {
		// Normal text isn't changed be this process, just return it as-is.
		return tok.getValue();
	}
	
	/**
	 * Evaluate the Italic production of the grammar.
	...
	private String evalItalic(Token tok) {
		...
		for (tok = lex.next();
			 (tok.getType() != Type.EOF) && (tok.getType() != Type.UNDERLINE);
			 tok = lex.next()) {
	...
	static String shuffle(String s) {
		...
		// e.g. "", "a", "b", "c"
		// (produces an extra empty string, but that won't hurt)
		String[] a = s.split("");
	...
	public static void main(String[] args) {
		Gallileo g = new Gallileo("I've discoverd that _Saturn has ears_. Suck it, Kepler!");
